...
class Index extends Component<{}, PageState> {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            name: '李俊辉',//收货人名字
            phone: '13790989809',//电话
            numberPlate: '13号',//门牌号
            addressList: [],//获取redux的地址列表
            address: { title: '', address: '' },//点击搜索列表的地址传入这里在页面上显示，保存时拿这里的值
    ...
    componentWillUnmount() {//关闭该页面时将选择的地址缓存清除
        // console.log('卸载页面')
    ...
    async save() { //保存
        let { name, phone, numberPlate, address, list, defaults } = this.state
        console.log(address)
        if(address.title === ''){
            showModerNoCancel('','请选择地址')
            return
        }
        let sexIndex = 0
        let defaultsIndex = 0
        ...
        const data = await getAddress.addAddress(arr,"POST")
        Taro.hideLoading()
        if (!data) return
        if(data.code!=0){
